...
@dataclass
class DeviceRelationshipEntry(BaseModel):
    """Device relationship."""
...
@dataclass
class BaseStatefulDeviceDescription(BaseModel, BaseDevice, ABC, Generic[DeviceState]):
    ...
    desiredState: Union[BaseStatefulDeviceState, DeviceState]  # The desired device state.
    ...
    state: Union[BaseStatefulDeviceState, DeviceState]  # The current device state.
...
@dataclass
class BaseManagedDeviceDescription(BaseStatefulDeviceDescription[DeviceState], BaseModel, ABC):
...
LightState = Union[BaseStatefulDeviceState, _LightState]


@dataclass
class LightDescription(BaseManagedDeviceDescription[LightState], BaseModel):
...
LockState = Union[BaseStatefulDeviceState, _LockState]


@dataclass
class LockDescription(BaseManagedDeviceDescription[LockState], BaseModel):
    """Description of lock."""
...
PartitionState = Union[BaseStatefulDeviceState, _PartitionState]


@dataclass
class PartitionDescription(BaseManagedDeviceDescription[PartitionState], BaseModel):
    ...
    invalidExtendedArmingOptions: list[
        ExtendedArmingOptions
    ]  # The extended arming option combinations that are invalid for each arming mode.
...
SensorState = Union[BaseStatefulDeviceState, _SensorState]


@dataclass
class SensorDescription(BaseManagedDeviceDescription[SensorState], BaseModel):
...
@dataclass
class TemperatureDeviceDescription(BaseManagedDeviceDescription[DeviceState], BaseModel, ABC):
...
ThermostatState = Union[BaseStatefulDeviceState, _ThermostatState]
...
@dataclass
class ThermostatDescription(TemperatureDeviceDescription[_ThermostatState], BaseModel):
    """Description of temperature device."""
...
@dataclass
class AlarmSystemDescription(BaseDevice, BaseModel):
