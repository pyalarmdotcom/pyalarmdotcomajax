...
from pyalarmdotcomajax.const import ItemEvent, ListenerCallbackT, ListenerRegistryCallbackRecord, OtpType
from pyalarmdotcomajax.devices import ApiDeviceT, BaseDevice, DeviceTypeSpecificData, TroubleCondition, UserProfile
from pyalarmdotcomajax.devices.partition import Partition
from pyalarmdotcomajax.devices.registry import AttributeRegistry, DeviceRegistry, DeviceType
...
def _unsubscribe(
    registry: dict[str, list[ListenerRegistryCallbackRecord]],
    device_id: str,
    callback: ListenerCallbackT,
    listener_name: str | None = None,
) -> None:
    """Unregister callback to be called when device state changes."""

    with contextlib.suppress(KeyError, ValueError):
        registry[device_id].remove(ListenerRegistryCallbackRecord(callback, listener_name))


async def _handle_server_errors(
    ...
    match status_str:
        case "423":  # Processing Error
            ...
            raise NotAuthorized(f"Failed to request {request_name}.")

        case "403" | "409":
            raise AuthenticationFailed(
                f"Failed to request {request_name}. {'Invalid anti-forgery token.' if status_str == '403' else 'Two factor authentication cookie is incorrect.'}"
            )

        case _:
            raise UnexpectedResponse(f"Unknown error while requesting {request_name}.")
...
class AdcResourceFetcher:
    """Fetches devices and device resources from Alarm.com."""

    def __init__(self, websession: aiohttp.ClientSession, ajax_headers: dict[str, str]):
        """Initialize."""
        self._websession = websession
        self._ajax_headers = ajax_headers

    async def async_device_type_present(
        self,
        controller: AlarmController,
        system_id: str,
        ...
        try:
            ...
            async with controller.websession.get(
                url=AttributeRegistry.get_endpoints(DeviceType.SYSTEM)["primary"].format(c.URL_BASE, system_id),
                headers=controller.ajax_headers,
            ) as resp:
                json_rsp = await resp.json()

                await _handle_server_errors(controller, "image sensors", resp.status, json_rsp)

                device_type_id = AttributeRegistry.get_type_id_from_devicetype(device_type)

                try:
                    return json_rsp["data"]["relationships"][device_type_id]["meta"]["count"] > 0
                except KeyError:
                    return False
...
class AlarmController:
    ...
    def __init__(
        ...
        self._partition_map: dict = {}  # Individual devices don't list their associated partitions. This map is used to retrieve partition id when each device is created.
        ...
        self._session_refresh_interval_ms: int = SESSION_REFRESH_DEFAULT_INTERVAL_MS
        self._keep_alive_url: str = KEEP_ALIVE_DEFAULT_URL
        ...
        self._listener_registry: dict[str, list[ListenerRegistryCallbackRecord]] = {}
    ...
    def two_factor_cookie(self) -> str | None:
        ...
        return (
            ...
            and (cookie := self._two_factor_cookie.get(LOGIN_TWO_FACTOR_COOKIE_NAME))
            else None
        )
    ...
    async def get(self, url: str) -> dict:
        """Make a GET request to Alarm.com."""

        try:
            async with self._websession.get(url=url, headers=self._ajax_headers) as resp:
                log.debug("Response status from Alarm.com: %s", resp.status)
                json_rsp = await resp.json()

                await _handle_server_errors("get", resp.status, json_rsp)
    ...
    async def async_update(self) -> None:
        ...
        if has_scenes:
            ...
            if not self._last_scene_update or (
                datetime.now() > self._last_scene_update + timedelta(minutes=SCENE_REFRESH_INTERVAL_M)
            ):
        ...
        for partition_raw in [
            ...
            partition_instance: BaseDevice = await self._async_update__build_hardware_device(
        ...
        for device_raw in raw_devices:
            try:
                device_instance: BaseDevice = await self._async_update__build_hardware_device(
    ...
    async def async_send_command(
        ...
        try:
            async with self._websession.post(url=url, json=msg_body, headers=self._ajax_headers) as resp:
                ...
                await _handle_server_errors("send_command", resp.status, json_rsp, retry_on_failure)
    ...
    def subscribe(self, device_id: str, callback: ListenerCallbackT, listener_name: str | None = None) -> partial:
        """Register callback to be called when device state changes."""

        self._listener_registry.setdefault(device_id, []).append(
            ListenerRegistryCallbackRecord(callback, listener_name)
        )

        return partial(_unsubscribe, self._listener_registry, device_id, callback, listener_name)

    def notify(self, device_id: str, event_type: ItemEvent, data: dict) -> None:  # ListenerCallbackT
        """Notify listeners."""

        for callback_fn, listener_name in self._listener_registry.get(device_id, []):
            try:
                log.debug("Notifying listener %s for device %s of event %s.", listener_name, device_id, event_type)
                callback_fn(device_id, event_type, data)
            except NameError:
                log.exception(
                    f"Failed to notify listener {listener_name} for device {device_id} of event {event_type}. The"
    ...
    async def async_login(
        ...
        try:
            ...
            async with self._websession.get(url=LOGIN_URL, cookies=None) as resp:
        ...
        try:
            ...
            async with self._websession.post(
                url=LOGIN_POST_URL,
                data={
                    LOGIN_USERNAME_FIELD: self._username,
                    LOGIN_PASSWORD_FIELD: self._password,
        ...
        async with self._websession.get(
            ...
            try:
                ...
                self._session_refresh_interval_ms = SESSION_REFRESH_DEFAULT_INTERVAL_MS
            ...
            try:
                ...
                self._keep_alive_url = KEEP_ALIVE_DEFAULT_URL
        ...
        try:
            async with self._websession.get(
                url=LOGIN_2FA_DETAIL_URL_TEMPLATE.format(c.URL_BASE, self._user_id),
                headers=self._ajax_headers,
            ) as resp:
                json_rsp = await resp.json()

            await _handle_server_errors("2FA requirements", resp.status, json_rsp, False)
    ...
    async def async_request_otp(self, method: OtpType | None) -> None:
        ...
        request_url = (
            ...
            else LOGIN_2FA_REQUEST_OTP_SMS_URL_TEMPLATE
        )
        try:
            async with self._websession.post(
    ...
    async def async_submit_otp(
        ...
        try:
            ...
            async with self._websession.post(
                url=LOGIN_2FA_POST_URL_TEMPLATE.format(c.URL_BASE, self._user_id),
                headers=self._ajax_headers,
        ...
        if suggested_device_name := json_rsp.get("value", {}).get("deviceName"):
            try:
                ...
                async with self._websession.post(
                    url=LOGIN_2FA_TRUST_URL_TEMPLATE.format(c.URL_BASE, self._user_id),
                    headers=self._ajax_headers,
        ...
        for cookie in self._websession.cookie_jar:
            if cookie.key == LOGIN_TWO_FACTOR_COOKIE_NAME:
    ...
    async def is_logged_in(self, throw: bool = False) -> bool:
        ...
        url = f"{c.URL_BASE[:-1]}{self._keep_alive_url}{KEEP_ALIVE_URL_PARAM_TEMPLATE.format(int(round(datetime.now().timestamp())))}"
    ...
    async def _reload_session_context(self) -> None:
        ...
        async with self._websession.post(
            url=KEEP_ALIVE_RENEW_SESSION_URL_TEMPLATE.format(c.URL_BASE, self._user_id),
            headers=self._ajax_headers,
    ...
    async def _async_get_active_system(self, retry_on_failure: bool = True) -> str:
        ...
        try:
            ...
            async with self._websession.get(
                url=ALL_SYSTEMS_URL_TEMPLATE.format(c.URL_BASE),
                headers=self._ajax_headers,
            ) as resp:
                json_rsp = await resp.json()

                await _handle_server_errors("active system", resp.status, json_rsp, retry_on_failure)
    ...
    async def _async_get_recent_images(self) -> dict[str, DeviceTypeSpecificData]:
        ...
        try:
            ...
            async with self._websession.get(
                url=ALL_RECENT_IMAGES_TEMPLATE.format(c.URL_BASE),
                headers=self._ajax_headers,
            ) as resp:
                json_rsp = await resp.json()
    ...
    async def _async_get_system(self, system_id: str, retry_on_failure: bool = True) -> list[dict]:
        ...
        try:
            ...
            async with self._websession.get(
                ...
                await _handle_server_errors("system", resp.status, json_rsp, retry_on_failure)
    ...
    async def _async_get_system_devices(self, system_id: str, retry_on_failure: bool = True) -> list[dict]:
        ...
        try:
            ...
            async with self._websession.get(
                url=ALL_DEVICES_URL_TEMPLATE.format(c.URL_BASE, system_id),
                headers=self._ajax_headers,
            ) as resp:
                json_rsp = await resp.json()
                ...
                await _handle_server_errors("system devices", resp.status, json_rsp, retry_on_failure)
